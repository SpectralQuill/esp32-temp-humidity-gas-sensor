...
function getLatestVersionedFile(
    proteusFolder: string = "proteus",
): string | null {
    try {
        ...
        const files = readdirSync(folderPath)
            .filter(
                (file) =>
                    file.toLowerCase().endsWith(".pdsprj") &&
                    VERSION_PATTERN.test(file),
            )
            .map((file) => ({
                name: file,
        ...
        if (files.length === 0) {
            console.log(
                `📁 No versioned .pdsprj files found in "${proteusFolder}/"`,
            );
            console.log(
                `   Looking for files matching pattern: *_v[number].pdsprj`,
            );
            return null;
        }
        ...
        files.forEach((file, index) => {
            ...
            console.log(
                `   ${index + 1}. ${file.name} (v${file.version}, modified: ${date})`,
            );
        });
