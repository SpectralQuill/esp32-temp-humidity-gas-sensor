...
async function testAPI() {
  ...
  try {
    ...
    const batchResult = await sensorDB.createReadings(
      { createdAt: new Date('2024-01-15T10:05:00Z'), readingType: 'temperature_c', value: 26.0 },
      { createdAt: new Date('2024-01-15T10:05:00Z'), readingType: 'humidity', value: 65.0 },
      { createdAt: new Date('2024-01-15T10:05:00Z'), readingType: 'gas', value: 42.0 }
    );
    ...
    console.log(`✅ Found: ${allReadings.temperaturesC.length} temps, ${allReadings.humidities.length} humidity, ${allReadings.gases.length} gas`);
