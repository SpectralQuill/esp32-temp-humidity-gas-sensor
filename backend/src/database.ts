...
export class SensorDatabase {
  ...
  async createReadings(
    ...
    const timestamp = temperatureC?.createdAt || humidity?.createdAt || gas?.createdAt || createdAt;
    
    if (temperatureC) {
      await this.createReading(timestamp, 'temperature_c', temperatureC.value);
  ...
  async getReadings(
    startDate: Date,
    endDate: Date
  ): Promise<{ temperaturesC: SensorReading[]; humidities: SensorReading[]; gases: SensorReading[] }> {
    ...
    return {
      temperaturesC: readings.filter(r => r.readingType === 'temperature_c'),
  ...
  async getTemperaturesC(startDate: Date, endDate: Date): Promise<SensorReading[]> {
    return this.getReadingsByType('temperature_c', startDate, endDate);
  }

  // API Method 5: Get humidity readings within date range
  async getHumidities(startDate: Date, endDate: Date): Promise<SensorReading[]> {
