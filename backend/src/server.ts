...
app.post('/api/reading', async (req, res) => {
  try {
    ...
    if (!createdAt || !readingType || value === undefined) {
      return res.status(400).json({ error: 'Missing required fields: createdAt, readingType, value' });
    }
    
    if (!['temperature_c', 'humidity', 'gas'].includes(readingType)) {
      return res.status(400).json({ error: 'Invalid readingType. Must be temperature_c, humidity, or gas' });
    }
...
app.post('/api/readings', async (req, res) => {
  try {
    ...
    if (!temperatureC && !humidity && !gas) {
      return res.status(400).json({ error: 'At least one reading (temperatureC, humidity, or gas) must be provided' });
    }
    
    const result = await sensorDB.createReadings(temperatureC, humidity, gas);
