...
const SERIAL_BAUD_RATE = process.env.SERIAL_BAUD_RATE ? parseInt(process.env.SERIAL_BAUD_RATE) : 115200;
const API_BASE_URL = process.env.API_BASE_URL || 'http://localhost:3000';
const MAX_RETRIES = process.env.SERIAL_MAX_RETRIES ? parseInt(process.env.SERIAL_MAX_RETRIES) : 3;
const RETRY_DELAY = process.env.SERIAL_RETRY_DELAY ? parseInt(process.env.SERIAL_RETRY_DELAY) : 5000;
const SERIAL_DATA_BITS = (process.env.SERIAL_DATA_BITS ? parseInt(process.env.SERIAL_DATA_BITS) : 8) as (8 | 7 | 6 | 5);
const SERIAL_PARITY = (process.env.SERIAL_PARITY || 'none') as ('none' | 'even' | 'odd' | 'mark' | 'space');
const SERIAL_STOP_BITS = (process.env.SERIAL_STOP_BITS ? parseInt(process.env.SERIAL_STOP_BITS) : 1) as (1 | 1.5 | 2);
...
class SerialListener {
  ...
  private async initialize(): Promise<void> {
    try {
      console.log(`🔗 Connecting to ${SERIAL_PORT} at ${SERIAL_BAUD_RATE} baud...`);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (latestReadings.temperatureC) {
          this.currentValues.temperature_c = latestReadings.temperatureC.value;
          console.log(`🌡️  Latest temperature from DB: ${latestReadings.temperatureC.value}°C`);
        } else {
        ...
        if (latestReadings.humidity) {
          ...
          console.log(`💧 Latest humidity from DB: ${latestReadings.humidity.value}%`);
        } else {
        ...
        if (latestReadings.gas) {
          ...
          console.log(`🔥 Latest gas from DB: ${latestReadings.gas.value}`);
        } else {
        ...
        this.hasInitialValues = Object.values(this.currentValues).some(val => val !== null);
        
      } else {
        console.warn('⚠️  Could not fetch latest readings from API, starting with empty values');
        this.setEmptyValues();
      }
      
    } catch (error) {
      console.error('❌ Error fetching latest values:', error);
      console.log('⚠️  Starting with empty values. Is the API server running?');
      this.setEmptyValues();
    }
  }
  ...
  private handleIncomingData(data: string): void {
    ...
    try {
      ...
      const sensorData: Esp32Data = {
        raw: rawData,
        timestamp,
        temperature_c: jsonData.temperature_c !== undefined ? parseFloat(jsonData.temperature_c) : undefined,
        humidity: jsonData.humidity !== undefined ? parseFloat(jsonData.humidity) : undefined,
        gas: jsonData.gas !== undefined ? parseFloat(jsonData.gas) : undefined
      };
  ...
  private processEsp32Data(sensorData: Esp32Data): void {
    ...
    if (sensorData.temperature_c !== undefined) {
      if (sensorData.temperature_c <= 40) {
        if (this.currentValues.temperature_c === null || 
    ...
    if (sensorData.humidity !== undefined) {
      if (sensorData.humidity >=0 && sensorData.humidity <= 100) {
        if (this.currentValues.humidity === null || 
    ...
    if (sensorData.gas !== undefined) {
      if (sensorData.gas >= 0 && sensorData.gas <= 100) {
        if (this.currentValues.gas === null || 
  ...
  private async sendToApi(payload: any, isInitialData: boolean = false): Promise<void> {
