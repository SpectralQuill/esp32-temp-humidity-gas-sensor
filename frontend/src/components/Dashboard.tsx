...
const Dashboard: React.FC = () => {
    ...
    const mergeSensorData = (
        ...
        const addReading = (timestamp: Date, type: 'temperatureC' | 'humidity' | 'gas', value: number) => {
            const existing = allReadings.find(r => 
        ...
        temperatureCData.forEach(reading => {
            addReading(new Date(reading.createdAt), 'temperatureC', reading.value);
        });
    ...
    const getCurrentStatus = () => {
        ...
        if (latestValues.temperatureC) {
            ...
            if (temp < TEMPERATURE_LEVELS.EXTREME_COLD || temp > TEMPERATURE_LEVELS.EXTREME_HOT) {
                status.temperature = temp < TEMPERATURE_LEVELS.EXTREME_COLD ? 'Extreme Cold' : 'Extreme Heat';
                hasDanger = true;
            } else if (temp < TEMPERATURE_LEVELS.DANGER_COLD || temp > TEMPERATURE_LEVELS.DANGER_HOT) {
                status.temperature = temp < TEMPERATURE_LEVELS.DANGER_COLD ? 'Too Cold' : 'Too Hot';
                hasDanger = true;
            } else if (temp < TEMPERATURE_LEVELS.COMFORT_LOW || temp > TEMPERATURE_LEVELS.COMFORT_HIGH) {
                status.temperature = temp < TEMPERATURE_LEVELS.COMFORT_LOW ? 'Cool' : 'Warm';
                hasWarning = true;
            }
        }
        ...
        if (latestValues.humidity) {
            ...
            if (humidity > HUMIDITY_LEVELS.DANGER_HUMID) {
                ...
                hasDanger = true;
            } else if (humidity > HUMIDITY_LEVELS.TOO_HUMID || humidity < HUMIDITY_LEVELS.TOO_DRY) {
                status.humidity = humidity > HUMIDITY_LEVELS.TOO_HUMID ? 'Humid' : 'Dry';
                hasWarning = true;
            }
        }
    ...
    return (
        ...
        <div className={`health-badge ${currentStatus.overall.toLowerCase()}`}>
        ...
        <div className={`value-card temperature ${currentStatus.temperature.toLowerCase().replace(' ', '-')}`}>
        <h3>🌡️ Temperature</h3>
        <div className="value">
        ...
        <div className={`value-card humidity ${currentStatus.humidity.toLowerCase().replace(' ', '-')}`}>
        <h3>💧 Humidity</h3>
        <div className="value">
        ...
        <div className={`value-card gas ${currentStatus.gas.toLowerCase().replace(' ', '-')}`}>
        <h3>🔥 Gas</h3>
        <div className="value">
        {latestValues.gas ? 
        ...
        tick={{ fontSize: 12 }}
        domain={[
            (dataMin: number) => Math.min(0, dataMin - 2), // Minimum of 0 or dataMin - 2
            (dataMax: number) => Math.max(39, dataMax + 2) // Maximum of 39 or dataMax + 2
        ]}
        />
        ...
        <Tooltip 
        formatter={(value: number | undefined) => value !== undefined ? [`${value.toFixed(0)}%`, 'Humidity'] : ['--', 'Humidity']}
        labelFormatter={(label) => `Time: ${label}`}
        />
        <Legend />
        <Area 
        ...
        <Tooltip 
        formatter={(value: number | undefined) => value !== undefined ? [`${value.toFixed(0)}%`, 'Gas'] : ['--', 'Gas']}
        labelFormatter={(label) => `Time: ${label}`}
        />
        <Legend />
        <Area 
